...
class ChapterRepositoryImpl(private val handler: DatabaseHandler) : ChapterRepository {
    ...
    override suspend fun getRecents(filterScanlators: Boolean, search: String, limit: Long, offset: Long, contentType: Long): List<MangaChapter> =
        handler.awaitList { chaptersQueries.getRecents(search, filterScanlators.toInt().toLong(), contentType, limit, offset, MangaChapter::mapper) }
    ...
    override suspend fun insertBulk(chapters: List<Chapter>): List<Chapter> {
