...
import eu.kanade.tachiyomi.databinding.SkeletonMangaCardBinding

private val logger = Logger.withTag("SkeletonMangaCardView")

/**
 * Skeleton placeholder for manga cards with high-performance shimmer animation.
 * Uses hardware-accelerated matrix transformation for 60fps smooth shimmer.
 * Shows only cover image placeholder, no text.
 */
class SkeletonMangaCardView @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = 0
) : FrameLayout(context, attrs, defStyleAttr) {
    
    private val binding: SkeletonMangaCardBinding
    ...
    init {
        binding = SkeletonMangaCardBinding.inflate(LayoutInflater.from(context), this, true)
        setWillNotDraw(false) // Enable onDraw for shimmer
        
        // Enable drawing on top of children (foreground layer)
        setWillNotDraw(false)
        
        logger.d { "✨ [SKELETON] SkeletonMangaCardView created" }
    }
    ...
    private fun setupShimmer(width: Int) {
        ...
        logger.d { "✨ [SKELETON] High-performance shimmer started (width: $width)" }
    }
    ...
    override fun onDetachedFromWindow() {
        ...
        logger.d { "✨ [SKELETON] SkeletonMangaCardView detached, shimmer paused" }
    }
}
