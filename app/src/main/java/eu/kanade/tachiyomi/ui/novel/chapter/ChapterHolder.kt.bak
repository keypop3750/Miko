...
class ChapterHolder(
    view: View,
    private val adapter: NovelDetailsAdapter,
) : BaseChapterHolder(view, adapter) {

    private val binding = NovelChaptersItemBinding.bind(view)
    ...
    fun bind(item: ChapterItem, novel: Novel) {
        val chapter = item.chapter
        val isLocked = item.isLocked
        itemView.transitionName = "details chapter ${chapter.id ?: 0L} transition"
        binding.chapterTitle.text =
            chapter.preferredChapterName(itemView.context, novel, adapter.preferences)

        binding.downloadButton.downloadButton.isVisible = !novel.isLocal() && !isLocked
        localSource = novel.isLocal()
