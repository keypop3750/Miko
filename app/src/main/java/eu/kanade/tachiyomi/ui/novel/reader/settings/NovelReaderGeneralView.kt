...
import eu.kanade.tachiyomi.ui.novel.reader.ReadingMode
...
class NovelReaderGeneralView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
    ...
    fun initGeneralPreferences() {
        // Disable manga-specific settings
        ...
        // Phase 4.4: Reading mode selector (DEFAULT, INFINITE_SCROLL, OVERSCROLL)
        binding.novelReadingMode.setEntries(
            ReadingMode.getDisplayStrings(context)
        )
        val currentMode = preferences.novelReadingMode().get()
        binding.novelReadingMode.setSelection(currentMode)
        binding.novelReadingMode.onItemSelectedListener = { position ->
            val newMode = ReadingMode.fromPrefValue(position)
            preferences.novelReadingMode().set(newMode.prefValue)
            activity.viewModel.setReadingMode(newMode)
        }
