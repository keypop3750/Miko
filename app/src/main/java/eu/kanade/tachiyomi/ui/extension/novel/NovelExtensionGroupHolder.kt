...
class NovelExtensionGroupHolder(view: View, adapter: FlexibleAdapter<IFlexible<RecyclerView.ViewHolder>>) :
    ...
    init {
        binding.extButton.setOnClickListener {
            (adapter as? NovelExtensionAdapter)?.listener?.onUpdateAllClicked(bindingAdapterPosition)
        }
        binding.extSort.setOnClickListener {
            (adapter as? NovelExtensionAdapter)?.listener?.onExtSortClicked(binding.extSort, bindingAdapterPosition)
        }
    }

    @SuppressLint("SetTextI18n")
    fun bind(item: NovelExtensionGroupItem) {
