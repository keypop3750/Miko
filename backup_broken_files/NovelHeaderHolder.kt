...
import eu.kanade.tachiyomi.databinding.NovelHeaderItemBinding
import eu.kanade.tachiyomi.source.SourceManager
import yokai.domain.novel.Novel
import eu.kanade.tachiyomi.source.model.SManga
import eu.kanade.tachiyomi.ui.novel.details.NovelDetailsController
import eu.kanade.tachiyomi.ui.novel.details.NovelDetailsAdapter
import eu.kanade.tachiyomi.ui.novel.details.NovelHeaderItem
...
class NovelHeaderHolder(
    view: View,
    private val adapter: eu.kanade.tachiyomi.ui.novel.details.NovelDetailsAdapter,
    startExpanded: Boolean,
    private val isTablet: Boolean = false,
) : BaseFlexibleViewHolder(view, adapter) {

    val binding: NovelHeaderItemBinding? = try {
        NovelHeaderItemBinding.bind(view)
    } catch (e: Exception) {
        null
    }
    ...
    init {
        ...
        with(binding) {
            ...
            novelSummary.setOnClickListener {
            ...
            novelSummary.setOnLongClickListener {
                if (novelSummary.isTextSelectable && !adapter.recyclerView.canScrollVertically(
                        -1,
                    )
                ) {
                    (adapter.delegate as NovelDetailsController).binding.swipeRefresh.isEnabled =
                        false
                }
                false
            }
            novelSummary.setOnTouchListener { _, event ->
                if (event.action == MotionEvent.ACTION_DOWN) {
                    view.requestFocus()
                }
                if (event.actionMasked == MotionEvent.ACTION_UP) {
                    hadSelection = novelSummary.hasSelection()
                    (adapter.delegate as NovelDetailsController).binding.swipeRefresh.isEnabled =
                        true
                }
                false
            ...
            shareButton.setOnClickListener { adapter.delegate.prepareToShareNovel() }
            favoriteButton.setOnClickListener {
                adapter.delegate.favoriteNovel(false)
            }
            favoriteButton.setOnLongClickListener {
                adapter.delegate.favoriteNovel(true)
                true
            }
            ...
            novelAuthor.setOnClickListener { view ->
                novelAuthor.text?.toString()?.let {
                    adapter.delegate.showFloatingActionMode(view as TextView, it)
                }
            }
            novelAuthor.setOnLongClickListener {
                novelAuthor.text?.toString()?.let {
                    adapter.delegate.copyContentToClipboard(it, MR.strings.author)
                }
                true
            }
            novelSummary.customSelectionActionModeCallback = adapter.delegate.customActionMode(novelSummary)
            applyBlur()
            novelCover.setOnClickListener { adapter.delegate.zoomImageFromThumb(coverCard) }
    ...
    private fun expandDesc(animated: Boolean = false) {
        ...
        if (binding.moreButton.visibility == View.VISIBLE || isTablet) {
            ...
            binding.novelSummary.maxLines = Integer.MAX_VALUE
            binding.novelSummary.setTextIsSelectable(true)
            setDescription()
            binding.novelGenresTags.isVisible = true
            ...
            binding.novelAuthor.maxLines = Integer.MAX_VALUE
            binding.novelSummary.requestFocus()
            if (animated) {
    ...
    private fun collapseDesc(animated: Boolean = false) {
        ...
        binding.novelSummary.setTextIsSelectable(false)
        binding.novelSummary.isClickable = true
        binding.novelSummary.maxLines = 3
        setDescription()
        binding.novelGenresTags.isVisible = isTablet
        binding.lessButton.isVisible = false
        binding.title.maxLines = 4
        binding.novelAuthor.maxLines = 2
    ...
    private fun setDescription() {
        if (binding != null) {
            val desc = adapter.controller.novelPresenter().novel.description
            binding.novelSummary.text = when {
                desc.isNullOrBlank() -> itemView.context.getString(MR.strings.no_description)
                binding.novelSummary.maxLines != Int.MAX_VALUE -> desc.replace(
                    Regex(
                        "[\\r\\n\\s*]{2,}",
    ...
    fun bindChapters() {
        val presenter = adapter.delegate.novelPresenter()
    ...
    fun bind(item: NovelHeaderItem) {
        ...
        setGenreTags(binding, novel)

        if (true /* novels only have author, no separate artist */) {
            binding.novelAuthor.text = novel.author?.trim()
        } else {
            binding.novelAuthor.text = listOfNotNull(novel.author?.trim(), null /* novels do not have artist field */?.trim()).joinToString(", ")
        }
        setDescription()

        binding.novelSummary.post {
            if (binding.subItemGroup.isVisible) {
                if (binding.novelSummary.lineCount < 3 && novel.genre.isNullOrBlank() &&
                    binding.moreButton.isVisible && novel.initialized
                ) {
                    expandDesc()
                    binding.lessButton.isVisible = false
        ...
        binding.novelSummaryLabel.text = itemView.context.getString(
            MR.strings.about_this_,
            novel.seriesType(itemView.context),
        )
        with(binding.favoriteButton) {
            icon = ContextCompat.getDrawable(
                ...
                when {
                    ...
                    novel.isFavorite -> R.drawable.ic_heart_24dp
            ...
            text = itemView.context.getString(
                when {
                    ...
                    novel.isFavorite -> MR.strings.in_library
                    else -> MR.strings.add_to_library
                },
            )
            checked(!item.isLocked && novel.isFavorite)
        ...
        with(binding.startReadingButton) {
            ...
            text = if (nextChapter != null) {
                ...
                context.getString(MR.strings.start_reading)
            }
        }
        ...
        binding.novelStatus.isVisible = novel.status != 0
        binding.novelStatus.text = (
    ...
    private fun setGenreTags(binding: NovelHeaderItemBinding, novel: Novel) {
        with(binding.novelGenresTags) {
            ...
            val amoled = adapter.delegate.novelPresenter().preferences.themeDarkAmoled().get()
            ...
            if (novel.genre.isNullOrBlank().not()) {
                (novel.genre?.split(",")?.map { it.trim() } ?: emptyList()).map { genreText ->
    ...
    fun clearDescFocus() {
        binding ?: return
        binding.novelSummary.setTextIsSelectable(false)
        binding.novelSummary.clearFocus()
    }
    ...
    fun updateColors(updateAll: Boolean = true) {
        ...
        val novel = adapter.presenter.novel
        with(binding) {
    ...
    fun updateCover(novel: Novel) {
        binding ?: return
        if (!novel.initialized) return
        val drawable = adapter.controller.binding.novelCoverFull.drawable
        ...
        binding.backdrop.loadNovel(novel) {
            placeholder(drawable)
            error(drawable)
            if (novel.isFavorite) networkCachePolicy(CachePolicy.READ_ONLY)
    ...
    fun expand() {
        ...
        if (!showMoreButton) {
            ...
            if (binding.novelSummary.maxLines != Integer.MAX_VALUE) {
                binding.moreButtonGroup.isVisible = !isTablet
            } else {
                binding.lessButton.isVisible = !isTablet
                binding.novelGenresTags.isVisible = true
