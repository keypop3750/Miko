...
import eu.kanade.tachiyomi.ui.novel.chapter.BaseChapterAdapter
...
class NovelDetailsAdapter(
    val controller: NovelDetailsController,
    ...
    val delegate: NovelDetailsInterface = controller
    ...
    override fun onCreateBubbleText(position: Int): String {
        ...
        return when (val scrollType = presenter.scrollType) {
            NovelDetailsPresenter.MULTIPLE_VOLUMES, NovelDetailsPresenter.MULTIPLE_SEASONS -> {
                val volume = ChapterUtil.getGroupNumber(chapter)
                if (volume != null) {
                    recyclerView.context.getString(
                        if (scrollType == NovelDetailsPresenter.MULTIPLE_SEASONS) {
                            MR.strings.season_
                        } else {
                            MR.strings.volume_
                        },
            ...
            NovelDetailsPresenter.TENS_OF_CHAPTERS -> recyclerView.context.getString(
                MR.strings.chapters_,
                get10sRange(chapter.chapter_number.toDouble()),
            )
    ...
    private fun get10sRange(value: Double): String {
    ...
    interface NovelHeaderInterface {
        fun coverColor(): Int?
        fun accentColor(): Int?
        fun novelPresenter(): NovelDetailsPresenter
        fun prepareToShareNovel()
        fun openInWebView()
        ...
        fun favoriteNovel(longPress: Boolean)
