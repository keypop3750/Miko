...
import eu.kanade.tachiyomi.util.novel.MangaUtil
...
import yokai.domain.novelchapter.interactor.GetAvailableScanlators
import yokai.domain.novelchapter.interactor.GetNovelChapter
import yokai.domain.novelchapter.interactor.UpdateChapter
import yokai.domain.history.interactor.GetHistory
import yokai.domain.library.custom.model.CustomMangaInfo
import yokai.domain.novel.interactor.GetManga
import yokai.domain.novel.interactor.UpdateManga
import yokai.domain.novel.models.MangaUpdate
...
//    val currentManga get() = currentMangaInternal.asStateFlow()
    ...
    fun onCreateLate() {
        ...
        val fetchMangaNeeded = !novel.initialized || manga.isLocal()
...
//        currentMangaInternal.update { manga }
    ...
    private suspend fun getChapters(queue: List<Download> = downloadManager.queueState.value) {
        val chapters = getChapter.awaitAll(novelId, isScanlatorFiltered()).map { it.toModel() }
        allChapters = if (!isScanlatorFiltered()) chapters else getChapter.awaitAll(novelId, false).map { it.toModel() }
    ...
    fun getChapterUrl(chapter: NovelChapter): String? {
        ...
        return chapterUrl.takeIf { !it.isNullOrBlank() }
            ?: try { source.getChapterUrl(novel, chapter) } catch (_: Exception) { null }
    }
     ...
     */
    fun getNextUnreadNovelChapter(): ChapterItem? {
        return chapterSort.getNextUnreadNovelChapter(chapters)
    }
    ...
    fun startDownloadingNow(chapter: NovelChapter) {
     ...
     */
    fun deleteNovelChapter(chapter: ChapterItem) {
    ...
    fun deleteChapters(chapters: List<ChapterItem>, update: Boolean = true, isEverything: Boolean = false) {
        ...
        if (isEverything) {
            downloadManager.deleteManga(novel, source)
        } else {
            downloadManager.deleteChapters(chapters, novel, source)
        }
    }

    suspend fun refreshNovelFromDb(): Manga {
        val dbManga = getManga.awaitById(novelId)!!
        setCurrentNovel(dbnovel)
        return dbManga
    }
    ...
    private suspend fun fetchNovelFromSource() {
        try {
            withIOContext {
                ...
                manga.copyFrom(networknovel)
                novel.initialized = true

                updateManga.await(manga.toNovelUpdate())
        ...
        try {
            withIOContext {
                ...
                if (added.isNotEmpty()) {
                    ...
                    view?.view?.context?.let { NovelShortcutManager.updateShortcuts(it) }
                }
    ...
    fun hideTitle(hide: Boolean) {
        ...
        manga.setFilterToLocal()
        presenterScope.launchNonCancellableIO { updateManga.await(NovelUpdate(novel.id!!, chapterFlags = manga.chapter_flags)) }
        if (mangaFilterMatchesDefault()) {
            manga.setFilterToGlobal()
        }
        view?.refreshAdapter()
    ...
    private suspend fun asyncUpdateMangaAndChapters(justChapters: Boolean = false) {
        if (!justChapters) updateManga.await(NovelUpdate(novel.id!!, chapterFlags = manga.chapter_flags))
        getChapters()
        withUIContext { view?.updateChapters() }
    }
    ...
    fun updateNovel(
        ...
        if (manga.isLocal()) {
            ...
            if (seriesType != null) {
                ...
                presenterScope.launchIO { updateManga.await(NovelUpdate(novel.id!!, viewerFlags = manga.viewer_flags)) }
            }
            novel.status = status ?: SManga.UNKNOWN
            LocalSource(downloadManager.context).updateMangaInfo(novel, lang)
            presenterScope.launchIO {
                updateManga.await(
                    NovelUpdate(
                        novel.id!!,
            ...
            launchNow {
                customMangaManager.saveMangaInfo(novel)
            }
        }
        if (uri != null) {
    ...
    private fun saveCover(directory: UniFile): UniFile {
        val cover = coverCache.getCustomCoverFile(novel).takeIf { it.exists() } ?: coverCache.getCoverFile(novel.posterUrl, !novel.inLibrary)
    ...
    fun removeTracker(trackItem: TrackItem, removeFromService: Boolean) {
        presenterScope.launch {
            withContext(Dispatchers.IO) {
                deleteTrack.awaitForManga(novel.id!!, trackItem.service.id)
